...
let usuarioConEmail = "lourg91@gmail.com"
//console.log (usuarioConEmail.length)
let contraseñaUsuario = "Mario1234"
...
//    Valide que el usuario y la contraseña coincidan con los datos de las variables,
//  estos datos tienen que ser iguales entre sí.

// En el caso de que no lo sean, devuelva un mensaje en forma de alerta, para cada uno de los casos: 
//"Su usuario es erróneo" o "Su contraseña es errónea".

//EN EL CASO DE QUE SE EQUIVOQQUE EN LOS DOS: "SU USUARIO Y CONTRASEÑA SO ERRÓNEOS".
...
const validarIngreso = ( usuario , contraseña) => {
    if (usuarioConEmail !== usuario && contraseñaUsuario===contraseña) {
         alert("Su usuario es erróneo")
    }else if  (usuarioConEmail === usuario && contraseñaUsuario !== contraseña) {
...
validarIngreso (prompt("usuario"), prompt("contraseña"))
...
const validarArroba = (nombre) =>{

    
    
    for ( let i = 0; i < nombre.length; i++){
        if(nombre[i].match(/[@]/) ){
            console.log("su mail es valido")
            break
...
validarArroba(prompt("para validar @ ingrese su usuario"))



// d) Valide si la contraseña del usuario tiene más de 8 caracteres.
// En el caso de que tenga más de ocho caracteres, no haga nada. 
//De lo contrario, envíe una alerta con el siguiente mensaje: 
//"Su contraseña es insegura."
...
validarContrasenia (prompt("su contraseña es segura? ingrese su contraseña"))


// e) Valide con un bucle que la contraseña al menos tenga un número.
// En el caso de que no tenga uno, envíe un prompt que exprese lo siguiente: 
...
const contraseniaConNumero = (password) =>{
    ...
    for( let i = 0; i < password.length; i++){
        if (password[i].match(/[0123456789]/)){
            ...
            let consulta =confirm("Su contraseña debe tener caracteres alfanuméricos. Desea cambiarla?")
            
            if (confirm){
                prompt("cambie el valor de la contraseña.")
                break
            }
...
contraseniaConNumero(prompt("su contraseña tiene numeros? ingrese su contraseña"))
...
// 3) Genere 2 prompt: uno que guarde el nombre del usuario
// y otro que permita al usuario ingresar una nota de menos 
//de 30 caracteres.


/////// PARA EL CASO BLOG

// TODOS LOS PUNTOS TIENEN QUE HACERSE CON FUNCIONES. 
//CADA PUNTO PUEDE EXPRESARSE COMO UNA FUNCIÓN. 
//LAS RESPONSABILIDADES DEBEN ESTAR SEPARADAS.
//UTILICE FUNCIONES QUE RECIBAN PARÁMETROS Y PASE 
//ARGUMENTOS EN LA LLAMADA.

// a) Verifique que la entrada del usuario sea menor a
// 30 caracteres. En el caso de que no lo sea,
// envíe una alerta con la siguiente frase: 
//"Esta entrada excede la cantidad de caracteres disponibles."
// b) En el caso de que el ingreso sea correcto, 
//envíe una alerta que muestre el nombre del usuario y 
//la nota que acaba de ingresar.
// c) Realice un bucle que envíe tres cuadros de diálogo 
//confirm que sean las solicitudes de amistad de nuevos 
//contactos. En cada iteración se debe poder visualizar 
//el mensaje: "El usuario ${i} te envió una solicitud de 
//amistad". 
